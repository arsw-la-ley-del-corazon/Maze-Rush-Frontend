...
const StyledWrapper = styled.div`
  ...
  position: relative;
  /* Variables ajustadas a la paleta (verde/neón + acentos azules/magentas) */
  .loader {
    --duration: 3s;
    --primary: #38f2a4; /* verde principal del gradiente en Home */
    --primary-light: #4fb3ff; /* azul claro acento */
    --primary-rgba: rgba(56, 242, 164, 0);
    width: 200px;
    height: 320px;
    position: relative;
    transform-style: preserve-3d;
  }
  @media (max-width: 480px) { .loader { zoom: 0.5; } }
  ...
  .loader .box div {
    background: linear-gradient(135deg,#38f2a4 0%, #48d43c 38%, #4fb3ff 68%, #c87dff 100%);
    ...
    box-shadow: 0 0 12px -2px rgba(56,242,164,0.6), 0 0 24px -4px rgba(79,179,255,0.5);
    border-radius:6px;
  }
  .loader .box div:before, .loader .box div:after {
    --rx: 90deg; --ry: 0deg; --z: 24px; --y: -24px; --x: 0;
    content:""; position:absolute; background:inherit; width:inherit; height:inherit;
    transform: rotateX(var(--rx)) rotateY(var(--ry)) translate(var(--x), var(--y)) translateZ(var(--z)); filter:brightness(var(--b,1.2));
    border-radius:6px;
  }
  .loader .box div:after { --rx:0deg; --ry:90deg; --x:24px; --y:0; --b:1.35; }
  /* Position pattern same as provided */
  ...
  /* Animations re-used original names */
  ...
  /* Keyframes (subset identical to original for behavior) */
