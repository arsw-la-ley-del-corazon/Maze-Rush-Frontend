...
const SignUpPage = () => {
    ...
    return (
        <Box sx={{
            ...
            <Button component={RouterLink} to="/" variant="text" startIcon={<ArrowBackIcon />} sx={{ position: 'absolute', top: 32, left: 32, color: 'rgba(255,255,255,0.7)', zIndex: 3, fontWeight: 'bold', textTransform: 'none', '&:hover': { color: '#fff' } }}>Volver</Button>
            <Paper elevation={0} sx={{
                width: '100%', maxWidth: 480, p: { xs: 3.5, sm: 5 }, position: 'relative', zIndex: 2, borderRadius: 4,
                ...
                <Typography variant="h4" fontWeight="bold" align="center" mb={1} sx={{ background: 'linear-gradient(90deg,#38f2a4 0%, #4ae46d 22%, #48d43c 42%, #4fb3ff 68%, #c87dff 92%)', WebkitBackgroundClip: 'text', color: 'transparent' }}>Crear cuenta</Typography>
                <Typography variant="body2" align="center" mb={3} sx={{ color: 'rgba(255,255,255,0.72)' }}>Regístrate para unirte a las partidas y competir.</Typography>
                <Box component="form" onSubmit={handleSubmit} sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                    <TextField label="Email" type="email" value={email} onChange={e => setEmail(e.target.value)} required fullWidth variant="outlined" InputProps={{ startAdornment: <EmailIcon sx={{ mr: 1, color: 'rgba(255,255,255,0.4)' }} /> }} sx={fieldSx} />
                        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
                            <TextField label="Contraseña" type="password" value={password} onChange={e => setPassword(e.target.value)} required fullWidth variant="outlined" InputProps={{ startAdornment: <LockIcon sx={{ mr: 1, color: 'rgba(255,255,255,0.4)' }} /> }} sx={fieldSx} />
                            <TextField label="Confirmar" type="password" value={confirm} onChange={e => setConfirm(e.target.value)} required fullWidth variant="outlined" InputProps={{ startAdornment: <LockIcon sx={{ mr: 1, color: 'rgba(255,255,255,0.4)' }} /> }} sx={fieldSx} error={confirm.length>0 && confirm!==password} helperText={confirm.length>0 && confirm!==password ? 'No coincide' : ' '} />
                        </Stack>
                    <FormControlLabel control={<Checkbox sx={{ color: 'rgba(255,255,255,0.4)', '&.Mui-checked': { color: '#38f2a4' } }} />} label={<Typography sx={{ color: 'rgba(255,255,255,0.7)' }}>Acepto términos</Typography>} />
                    <Button type="submit" disabled={loading || !email || !password || password !== confirm} fullWidth sx={submitButtonSx}>
                ...
                <Typography align="center" variant="body2" sx={{ color: 'rgba(255,255,255,0.7)' }}>
                    ¿Ya tienes cuenta? <Button component={RouterLink} to="/login" variant="text" sx={{ color: '#4fb3ff', fontWeight: 'bold', textTransform: 'none', '&:hover': { color: '#38f2a4' } }}>Inicia sesión</Button>
                </Typography>
